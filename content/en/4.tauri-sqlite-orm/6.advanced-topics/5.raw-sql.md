...
const verifiedUsers = await db.select(users)
  .where(sql`json_extract(${users.metadata}, '$.is_verified') = true`)
  .all();
```
...
This sends a single `UPDATE users SET login_count = login_count + ? WHERE id = ?` query (with params `[1, 1]`), ensuring the operation is safe even with concurrent requests.
