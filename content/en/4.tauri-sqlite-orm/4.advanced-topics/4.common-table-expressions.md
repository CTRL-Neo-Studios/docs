...
2. The `query` you pass to `.as()` must be a query object: `{ sql: string; params: any[] }`. You can generate this using the [[4.5 - Raw SQL|`sql` template literal]] or by using another query builder's `.build()` method.
