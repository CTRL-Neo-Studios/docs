...
- The **argument** to the column helper (`'full_name'`) is the column name in the database. The recommended convention for SQL is **snake_case**.
...
export const posts = sqliteTable('posts', {
  ...
  authorId: integer('author_id')
    .notNull()
    .references(users, users.id), // <-- This creates the link
});
```

Don't worry about all the modifiers like `.primaryKey()` and `.references()` just yet. They are covered in detail on the next page, [Column Types & Modifiers.](2.column-types-and-modifiers.md)